...
public static class ExtendedInput
{
    ...
    private static readonly Dictionary<int, KeyState> keyStates = new Dictionary<int, KeyState>();
    ...
    private static void GetKeyCurrentFrame(int keyCode, out bool isPressed, out bool didChange)
    {
        ...
        KeyState keyState;
        if (!keyStates.TryGetValue(keyCode, out keyState)) keyStates[keyCode] = keyState = new KeyState();
    ...
    {
#if EXTENDED_INPUT_WINDOWS
        var result = BitConverter.GetBytes(GetAsyncKeyState(keyCode));
    ...
    ///     <c>true</c> if the specified key is held; otherwise <c>false</c>.
    /// </returns>
    public static bool GetKey(ExtendedKeyCode keyCode)
    {
        return GetKey((int)keyCode);
    }
    ...
    ///     <c>true</c> if the specified key was pressed on the current frame; otherwise <c>false</c>.
    /// </returns>
    public static bool GetKeyDown(ExtendedKeyCode keyCode)
    {
        return GetKeyDown((int)keyCode);
    }
    ...
    ///     <c>true</c> if the specified key was released on the current frame; otherwise <c>false</c>.
    /// </returns>
    public static bool GetKeyUp(ExtendedKeyCode keyCode)
    {
        return GetKeyUp((int)keyCode);
    }
